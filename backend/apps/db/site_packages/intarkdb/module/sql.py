    ...
    def __init__(self, network):
        if network == False:
            if sys.platform.startswith('linux'):
                ...
                self._intarkdb_sql_clib = CDLL("libintarkdb.dll")
            else:
        ...
        # if network == True: 
            ...
            if sys.platform.startswith('linux'):
                ...
                self._intarkdb_sql_clib = CDLL("libnetwork_client_dynamic.dll")
            else:
                raise OperationalError()
        ...
        if network == False:
            ...
            self._intarkdb_sql_clib.intarkdb_expanded_sql.argtypes = (POINTER(INTARKDB_PREPARED_STATEMENT),)
            self._intarkdb_sql_clib.intarkdb_expanded_sql.restype = c_char_p
        # if network == True:
        else:
            # init cdll function signature   
            ...
            self._intarkdb_sql_clib.intarkdb_client_connect_with_timeout.argtypes = (c_char_p, c_int, c_char_p, c_char_p, c_char_p, POINTER(POINTER(INTARKDB_DATABASE)), c_int, c_int)
            self._intarkdb_sql_clib.intarkdb_client_connect_with_timeout.restype = c_int
...
class SQL:
    ...
    def open_db(self, path):
        ...
        if ret != SQL_SUCCESS:
            raise InterfaceError(ret, "open database failed")
    ...
    def intarkdb_client_connect(self, database, host, port, user, password, connect_timeout = None, socket_timeout = None):
        db_ptr = POINTER(INTARKDB_DATABASE)()
        if(connect_timeout == None and socket_timeout == None):
            ...
            ret = self._dynamic.get_intarkdb_sql_clib().intarkdb_client_connect_with_timeout(
                host.encode(), port, database.encode(), user.encode(), password.encode(), byref(db_ptr), connect_timeout, socket_timeout)
    ...
    def intarkdb_expanded_sql(self, prepared_statement):
        return self._dynamic.get_intarkdb_sql_clib().intarkdb_expanded_sql(prepared_statement)
