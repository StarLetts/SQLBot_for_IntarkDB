...
class Connection:
    ...
    def __init__(self, database = ".", host = None, port = None, user = None, password = None, connect_timeout = None, socket_timeout = None):
        self._path = database   
        self._conn = None
        self._kv = False
        ...
        with Connection.CONNECTION_LOCK:
            ...
            if (self._network == True):
                ret ,self._conn = self.__sql.intarkdb_client_connect(
                        self._path, self._host, self._port, self._user, self._password, self._connect_timeout, self._socket_timeout)
